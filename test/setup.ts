...
export interface TestEnv {
  // happy-dom's Window is typed with its own DOM classes, which aren't
  // assignable to lib.dom types used by src/. Expose it as a lib.dom window.
...
export const setupTestEnv = (): TestEnv => {
  ...
  const window = happyWindow as unknown as Window & typeof globalThis;
  ...
  if (
    typeof (window.Element.prototype as unknown as Record<string, unknown>)
  ...
  try {
    if (
      typeof (globalThis.Element as typeof Element | undefined)?.prototype !==
      'undefined'
    ) {
      const proto = (globalThis.Element as typeof Element)
  ...
  const elemProto = window.Element.prototype as unknown as Record<
    string,
    unknown
  >;
  if (
    typeof elemProto.getClientRects !== 'function' ||
    (elemProto.getClientRects as () => ArrayLike<unknown>)().length === 0
  ) {
  ...
  const htmlProto = window.HTMLElement.prototype as unknown as Record<
    string,
    unknown
  >;
  Object.defineProperty(htmlProto, 'offsetParent', {
    ...
    get() {
      ...
      return (
        self.parentElement ??
        (self === window.document.body ? window.document.body : null)
      );
    },
  });
